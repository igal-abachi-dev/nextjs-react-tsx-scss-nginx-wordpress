...
export const AuthApi = {
  ...
  login(userName: string, password: string, cb) {
    const encodedLogin = { challenge: '' };

    // 4-5 sec login , protects against brute force
    _api_post('/Login', encodedLogin, cb);
  },

  refreshToken(refreshToken: string, cb) {
    const refToken = { refreshToken: refreshToken || '' };
    _api_post('/RefreshToken', refToken, cb);
  },


  resetPasswordGenCode(userName: string, cb) {
